...
Note: `LogSeverity` values here are 0 (for low), 1 (for medium) and 2 (for high).

- Example for `LOGSTASH_SERVER`:
...
It is recommended to use `LOGSTASH_SERVER` option, because it is far easier to parse (i.e. JSON) and attribute names, which should be the same as those from Maltrail's UI.
