...
class ColorizedStream:
    def __init__(self, original):
        ...
        # JA3/JA4 sit next to UA on purpose: same idea (what the client looks like), one layer down
        self._type_colors = {"DNS": BACKGROUND.BLUE, "UA": BACKGROUND.MAGENTA, "IP": BACKGROUND.RED, "URL": BACKGROUND.YELLOW, "HTTP": BACKGROUND.GREEN, "IPORT": BACKGROUND.RED, "PATH": BACKGROUND.CYAN, "PORT": BACKGROUND.DARK_GRAY, "CERT": BACKGROUND.OLIVE, "JA3": BACKGROUND.LIGHT_MAGENTA, "JA4": BACKGROUND.LIGHT_MAGENTA}
    ...
    def write(self, text):
        ...
        if text.startswith('"2'):
            text = re.sub(r"(TCP|UDP|ICMP) ([A-Z0-9]+)", lambda match: "%s %s%s%s" % (match.group(1), self._type_colors.get(match.group(2), COLOR.WHITE), match.group(2), COLOR.RESET), text)
