...
SUSPICIOUS_HTTP_REQUEST_REGEXES = (
    ...
    ("potential remote code execution", r"\$_(REQUEST|GET|POST)\[|xp_cmdshell|shell_exec|exec_code|\bping(\.exe)? -[nc] \d+|timeout(\.exe)? /T|tftp -|wget http|curl -O|sh /tmp/|touch /tmp/|cmd\.exe|/bin/(ba)?sh\b|2>&1|\b(cat|ls) /|chmod [0-7]{3,4}\b|chmod +x\b|nc -l -p \d+|>\s*/dev/null|-d (allow_url_include|safe_mode|auto_prepend_file)|jndi:(corba|dns|http|iiop|n(d|i)s|ldap[s]?|rmi):?|base64:JHtqbmRp|ipconfig|net (config|view)|nltest|netsh firewall|\$\{IFS\}|getRuntime\(\)\.exec\("),
    ("potential directory traversal", r"(\.{2,}[/\\]+){3,}|\/etc\/([\/security\/]){0,}(auth(.dir|.pag)?|group|(master.)?passwd(.adjunct|.dir|.pag)?|shad(o|p)w|issue|hostname|hosts|sudoers|udb|tcb)|[/\\](boot|system|win)\.ini|[/\\]system32\b|%SYSTEMROOT%"),
