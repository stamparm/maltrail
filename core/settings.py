...
SUSPICIOUS_HTTP_REQUEST_REGEX = r"(?i)(information_schema|/\*.*\*/|floor\(rand\(|order by \d+|sysdatabases|(\.\./){3,}|\bselect\b.*\bfrom\b.*\bwhere\b|\bselect \w+ from \w+|<script|\balert\(|xp_cmdshell|/etc/passwd|<\?php|boot\.ini|\bsleep\(|\bconvert\(|varchar|\bunion\s+(all\s+)?select\b)"
