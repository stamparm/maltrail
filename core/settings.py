...
SUSPICIOUS_HTTP_REQUEST_REGEXES = (
    ("sql injection", r"information_schema|sysdatabases|floor\(rand\(|ORDER BY \d+|\bUNION\s+(ALL\s+)?SELECT\b|\b(UPDATEXML|EXTRACTVALUE)\(|\bCASE[^\w]+WHEN.*THEN\b|\bWAITFOR[^\w]+DELAY\b|\bCONVERT\(|VARCHAR\(|\bCOUNT\(\*\)|\b(pg_)?sleep\(|\bSELECT\b.*\bFROM\b.*\b(WHERE|GROUP|ORDER)\b|\bSELECT \w+ FROM \w+|\b(AND|OR|SELECT)\b.*/\*.*\*/|/\*.*\*/.*\b(AND|OR|SELECT)\b|\b(AND|OR)[^\w]+\d+['\") ]?[=><]['\"( ]?\d+"),
...
WORST_ASNS = {}
CLOUDFLARE_RANGES = {}
WHITELIST_HTTP_REQUEST_PATHS = ("fql", "yql", "ads", "../images/", "../themes/", "../design/", "../scripts/", "../assets/", "../core/", "../js/", "/gwx/", "/pub/softpaq/")
...
def read_cloudflare_ranges():
    _ = os.path.abspath(os.path.join(ROOT_DIR, "misc", "cloudflare.txt"))
    if os.path.isfile(_):
        with open(_, "r") as f:
            for line in f:
                line = line.strip()
                if not line or line.startswith('#'):
                    ...
                    key = line.split('.')[0]
                    if key not in CLOUDFLARE_RANGES:
                        CLOUDFLARE_RANGES[key] = []
                    prefix, mask = line.split('/')
                    CLOUDFLARE_RANGES[key].append((addr_to_int(prefix), make_mask(int(mask))))

if __name__ != "__main__":
    read_whitelist()
    read_ua()
    read_worst_asn()
    read_cloudflare_ranges()
