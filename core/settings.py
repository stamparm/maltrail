...
SUSPICIOUS_HTTP_REQUEST_REGEXES = (
    ...
    ("potential remote code execution", r"\$_(REQUEST|GET|POST)\[|xp_cmdshell|shell_exec|\bping(\.exe)? -[nc] \d+|timeout(\.exe)? /T|\/shell\?(wget|curl|tftp)|cmd=(wget|curl|tftp)|tftp -|wget http|curl -O|sh /tmp/|cmd\.exe|/bin/(ba)?sh\b|2>&1|\b(cat|ls) /|chmod [0-7]{3,4}\b|chmod +x\b|nc -l -p \d+|>\s*/dev/null|-d (allow_url_include|safe_mode|auto_prepend_file)|\$\{IFS\}"),
