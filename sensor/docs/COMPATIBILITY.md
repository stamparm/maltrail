...
(`tools/parity.py`). See `docs/REPORT.md` for that run. The corpus is now 40 cases; three of them
(`udp_malware_dst`, `dns_same_socket_burst`, `trail_under_whitelist_parent`) pin the deliberate
divergences in §2, differences 20-22, and are checked in **both** directions — the expected
Rust-only event must be present, and its absence fails the case. Run
`python3 sensor/tools/parity.py` for the current result.
