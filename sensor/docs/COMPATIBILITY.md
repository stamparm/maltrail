...
(`tools/parity.py`). See `docs/REPORT.md` for that run. The corpus is now 39 cases; two of them
(`udp_malware_dst`, `dns_same_socket_burst`) pin the deliberate divergences in §2, differences 20
and 21, and are checked in **both** directions — the expected Rust-only event must be present, and
its absence fails the case. Run `python3 sensor/tools/parity.py` for the current result.
