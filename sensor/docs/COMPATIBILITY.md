...
code (`tools/gen_vectors.py`), or the differential replay harness (`docs/COMPATIBILITY.md`).

Last recorded parity result, on the then-36-case corpus: **36/36 cases, 0 event differences**
(`docs/COMPATIBILITY.md`). See `docs/REPORT.md` for that run. The corpus is now 42 cases; five of them
...
and its absence fails the case. `tests/replay.rs` asserts every corpus expectation on each run.
...
* Verified: `tests/meta.rs` (schema, storage class, merge, junk filter, prune, failure
  handling) and `docs/COMPATIBILITY.md`, which now replays every corpus case through both sensors with
...
* `DLT_OFFSETS` for this platform, the `DLT_RAW` / `DLT_PPP` / `DLT_NULL` special cases, one
  802.1Q/802.1ad tags walked (see §2.24), IPv4/IPv6 EtherType gate.
* The unknown-datalink offset heuristic and its two-packets-agree learner.
* IPv4 (including IHL > 5), non-first fragments skipped, IPv6 with its extension-header chain
  walked to the transport header (see §2.25), TCP, UDP, ICMP and ICMPv6 echo-request gating, and
  the generic `IPPROTO_LUT` branch. Encapsulated packets are unwrapped rather than matched only on
  their outer header (see §2.27).
...
| --- | --- | --- | --- |
| 1 | **Offline timestamps default to the pcap record time.** `sensor.py` on Python 3 substitutes wall-clock time for every packet (a `pcapy-ng` workaround at `sensor.py:1524`). | Using the real timestamp is correct, and is what live capture and Python 2 do. `core/testing.py` itself skips the counting heuristics offline on Python 3 for this reason. | The sensor detects the timing heuristics (port/UDP/web/infection scan) during offline replay, which `sensor.py` cannot. Burst suppression, the log-throttle bucket and the hourly resets also behave correctly instead of collapsing. Pass `--timestamps wallclock` for byte-comparable parity runs; `docs/COMPATIBILITY.md` does this by default. |
...
| 5 | **NXDOMAIN name bundles are sorted.** Python joins a `set`, so its order is arbitrary *and varies between runs* (string hashing is randomised per process). | Determinism. | The trail text `(a,b,c).example.com` lists the same names in a stable order. `docs/COMPATIBILITY.md` sorts both sides before comparing — the only value-level normalisation it performs. |
...
| 24 | **Stacked VLAN tags are followed.** `sensor.py` skips one 802.1Q tag and drops anything QinQ. | A double-tagged frame is ordinary traffic at a carrier or datacentre edge, and dropping it means the sensor is blind on exactly the links most worth watching. | `0x8100`, `0x88a8` and `0x9100` are walked up to `MAX_VLAN_TAGS` (4); a deeper stack is dropped rather than chased, so a crafted tag chain cannot make the parser walk forever. Pinned by tests in `src/packet/dlt.rs`. |
| 25 | **IPv6 extension headers are traversed to reach the transport header.** `sensor.py` takes Next Header as the protocol and stops, so a packet carrying a Hop-by-Hop, Routing or Destination-Options header reports protocol 0/43/60. | That is an evasion primitive, not a parity detail. With the transport header never located, no port, DNS name, HTTP host or TLS SNI is read - eight bytes hid a packet from every payload-derived trail. Verified before the fix: a DNS query for a listed domain behind one Hop-by-Hop header produced zero events. | Hop-by-Hop, Routing, Fragment, AH, Destination-Options, Mobility, HIP and Shim6 are stepped over, up to `MAX_IPV6_EXT_HEADERS` (8); AH is measured in 4-octet units per RFC 4302, the rest in eights. ESP terminates the walk (the payload is encrypted). A non-first fragment stops the walk but is still reported, so the IP match it always made is not lost. Pinned by `ipv6_extension_headers_do_not_hide_the_payload` and `ipv6_port_trails_survive_an_extension_header` in `tests/detection.rs` plus five tests in `src/packet/mod.rs`. |
| 26 | **`CAPTURE_FANOUT_MODE source` — source-affine distribution with no `sensor.py` counterpart, and the default whenever `CAPTURE_WORKERS` > 1.** Python's `_src_hash` tried to keep one host's evidence on one worker, but the kernel has no source-only fanout mode, so it never did what its name says. | The scan heuristics count per source and `PACKET_FANOUT_HASH` splits per flow, which is why multi-worker capture loses alerts (§2.3). A classic-BPF program attached with `PACKET_FANOUT_CBPF` hashes the source address alone, so every packet a host sends reaches one worker. cBPF rather than eBPF deliberately: it goes in through `setsockopt`, so there is no `bpf()` syscall, no `CAP_BPF`, no libbpf and no compiled object to ship — the privileges are the ones capture already needs. | Heuristic survival goes from 91/87/66% to **100% at 2, 4 and 8 workers**, measured over the corpus by `source_affine_fanout_keeps_the_scan_heuristics`; exact detections are unchanged. The program reads the IP version and falls back to an EtherType lookup (following one 802.1Q tag), so it is correct whether the kernel presents the network header or the link-layer header — both layouts are executed by an interpreter in `capture::srcfanout`'s tests. Needs Linux 4.5+; a kernel that refuses the program fails the join loudly rather than demuxing everything to worker 0. **Verified against a live kernel** (`source_affine_fanout_actually_separates_sources_in_the_kernel`, root only): 64 spoofed sources sending four packets each on varying source ports through a real 4-worker group landed **64 packets on every worker with no source split**. That second half matters on its own — a program the kernel accepts but cannot read the source from loads cleanly and funnels everything to worker 0, which is indistinguishable from success unless the distribution is counted. |
| 27 | **Tunnelled traffic is decapsulated: VXLAN, GENEVE, GRE, ERSPAN and IP-in-IP.** `sensor.py` matches the outer header and stops, so an overlay's contents are invisible to it. | A SPAN feed at a datacentre or corporate border is usually an overlay - the addresses on the mirror port are tunnel endpoints, which is the operator's own infrastructure, and every host worth watching is inside. Matching only the outer header means no inner IP, no port, no DNS name, no HTTP host and no TLS SNI for any of them. Verified before the fix: a DNS query for a listed domain inside VXLAN, GENEVE, GRE and ERSPAN produced zero events. | ADDITIVE - the outer header is still matched exactly as before, so a listed tunnel endpoint still fires, and the inner packet is then processed in its own right. Both VXLAN ports (4789 and Linux's 8472), GENEVE with options, all eight GRE flag combinations, ERSPAN II and III, and inner frames carrying VLAN tags. Nesting is capped at `MAX_TUNNEL_DEPTH` (3) and the walk is iterative rather than recursive, so the per-packet entry point stays inlinable. A claimed inner header that does not begin with an IP version nibble is refused, which is what stops a scan of UDP/4789 having 16 arbitrary bytes read as addresses and reported as a detection. `maltrail_packets_decapsulated_total` counts what was unwrapped: zero on an overlay feed means the sensor is only seeing tunnel endpoints. Fuzzed with tunnelled seeds (`fuzz_parsers.rs`), since the GRE flag words and GENEVE `opt_len` are attacker-controlled lengths that become an offset the packet path then parses at. |
