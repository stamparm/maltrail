...
Current parity result on the shipped 36-case corpus: **36/36 cases, 0 event differences**
(`tools/parity.py`). See `docs/REPORT.md` for the full run.
...
| 15 | **Capture workers default to `PROCESS_COUNT`.** | One Rust worker is the equivalent of one `sensor.py` worker process, and the throttle keeps state per worker — so defaulting to `CAPTURE_FANOUT` (commented out in the shipped `maltrail.conf`) meant *one* worker against `sensor.py`'s sixteen, and therefore ~1/16 of the log lines for the same traffic. | `CAPTURE_WORKERS` still overrides. If the kernel refuses `PACKET_FANOUT` for the device, the sensor drops to a single worker with a loud warning rather than opening N duplicating sockets. |
...
| 18 | **CPU affinity (`schedtool`) is not set.** | `DISABLE_CPU_AFFINITY` exists because the Python heuristic caused load problems, and with kernel fanout the kernel already spreads work. | Pin with `taskset`/systemd `CPUAffinity=` if wanted. |
