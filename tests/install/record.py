...
def page(rows):
    ...
    body = [
        ...
        "linked at load time and `wpcap.dll` ships with the Npcap driver, which has no unattended "
        "installer - but the installer is an NSIS archive, so the userspace library can be "
        "extracted from it without installing anything, mingw-w64 cross-compiles the sensor, and "
        "Wine runs it. `sensor/tools/check_windows.sh` does all of that: the Windows sensor's "
        "entire unit suite, its `-T` self-test against the shipped `maltrail.conf`, every pcap in "
        "the corpus replayed through both the Windows and the native binary with the detections "
        "compared byte for byte, and the server answering `/ping` under a real Windows Python. "
        "The first run of it found four bugs that compiling could not.",
        "",
        "What that leaves untested is live capture, which needs the kernel driver and a real "
        "Windows machine. On one, `python3 tests/install/record.py record --windows <label>` "
        "produces a row the same way every other row here was produced. It is not filled in from "
        "a runner that could not have tested it.",
        "",
        "## Rows",
        "",
